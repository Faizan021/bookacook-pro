...
import {
  ...
  addBlackoutDate,
  removeBlackoutDate,
} from "@/lib/vendor/blackout.functions";
    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">
          Mark dates when you are fully booked or unavailable to prevent customers from requesting
          these dates.
        </p>
      </div>
            ...
            <div className="surface-card p-4 rounded-2xl bg-cream/10 border border-[#eadfce]/45 text-left space-y-3 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-250">
              ...
              <div className="space-y-2">
                ...
                <Button
                  ...
                  disabled={adding}
                  className="w-full rounded-full bg-forest text-xs py-2 h-9 font-semibold text-white"
                >
            ...
            {blackoutDates.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-muted-foreground text-center space-y-2">
                ...
                <p className="text-[10px] text-muted-foreground max-w-[200px]">
                  Select dates on the calendar on the left to mark your unavailable days.
                </p>
              </div>
            ) : (
              <div className="grid gap-2 max-h-[350px] overflow-y-auto pr-1">
                {blackoutDates.map((b: any) => (
                  <div
                    key={b.id}
                    className="flex items-center justify-between p-3 rounded-xl border border-border/40 bg-cream/5 hover:border-forest/35 transition"
                  >
                    <div>
                      <p className="font-mono text-xs font-bold text-forest">
                        {new Date(b.blackout_date).toLocaleDateString()}
                      </p>
                      {b.reason && (
                        <p className="text-[10px] text-muted-foreground mt-0.5">{b.reason}</p>
                      )}
                    </div>
                    <Button
                      variant="ghost"
