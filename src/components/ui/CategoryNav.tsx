...
const getCategoryIcon = (category: string) => {
  ...
  if (cat.includes("drink") || cat.includes("getränk") || cat.includes("beverage") || cat.includes("getränke")) return "🥤";
  ...
  if (cat.includes("antipasti") || cat.includes("starter") || cat.includes("vorspeise")) return "🧀";
  if (cat.includes("side") || cat.includes("beilage")) return "🍟";
  if (cat.includes("wein") || cat.includes("wine")) return "🍷";
  if (cat.includes("meat") || cat.includes("fleisch") || cat.includes("steak") || cat.includes("grill")) return "🥩";
...
export function CategoryNav({ categories, activeCategory, onSelect }: CategoryNavProps) {
  ...
  return (
    <div className="overflow-hidden border-b border-[oklch(0.85_0.05_152)] pb-4 mt-6" ref={emblaRef}>
        ...
        {categories.map((cat) => {
          ...
          return (
            <button
              ...
              `}
            >
              <div className={`grid h-10 w-10 place-items-center rounded-full text-2xl ${isActive ? 'bg-white/20' : 'bg-mint'}`}>
                {getCategoryIcon(cat)}
              </div>
