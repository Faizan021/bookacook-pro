      ...
      {auditResult && (
        <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm mb-6">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Scanned {auditResult.totalAudited} records. Found {auditResult.demotedRecords.length}{" "}
            violations.
          </p>
          {auditResult.demotedRecords.length > 0 && (
            <ul className="space-y-2 max-h-64 overflow-y-auto">
              {auditResult.demotedRecords.map((r, i) => (
                <li
                  key={i}
                  className="flex flex-col text-sm p-3 bg-red-50 border border-red-100 rounded-lg"
                >
                  <span className="font-semibold text-red-900">{r.title}</span>
                  <span className="text-red-700 text-xs">
                    Demoted from <strong className="uppercase">{r.previousStatus}</strong> to{" "}
          ...
          <button
            ...
            className="mt-4 px-3 py-1.5 bg-gray-100 hover:bg-gray-200 rounded-lg text-sm font-medium"
          >
            Dismiss
          </button>
        </div>
                ...
                <div className="flex flex-col gap-2 w-full">
                  ...
                  <div className="flex items-center justify-between w-full">
                    <div className="flex items-center gap-2">
                      <span
                        className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${statusColors[selectedDraft.status]}`}
                      >
                        {selectedDraft.status.replace("_", " ")}
                      </span>
                      <span className="text-xs text-gray-500">
                        Target:{" "}
                        <strong className="text-gray-900">{selectedDraft.target_keyword}</strong>
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      {selectedDraft.status === "draft" && (
                        <>
                          <button
                            onClick={() =>
                              statusMutation.mutate({ id: selectedDraft.id, status: "rejected" })
                            }
                          ...
                          <button
                            onClick={() =>
                              statusMutation.mutate({ id: selectedDraft.id, status: "in_review" })
                            }
                            className="px-3 py-1.5 text-xs font-semibold bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-lg flex items-center gap-1"
                          >
                            <Eye className="w-3 h-3" /> Request Review
                          </button>
                        </>
                      )}
                      {selectedDraft.status === "in_review" && (
                        <>
                          ...
                          <button
                            onClick={() =>
                              statusMutation.mutate({ id: selectedDraft.id, status: "approved" })
                            }
                            className="px-3 py-1.5 text-xs font-semibold bg-green-50 text-green-700 hover:bg-green-100 rounded-lg flex items-center gap-1"
                          >
                      ...
                      {selectedDraft.status === "approved" && (
                        <>
                          <button
                            onClick={() =>
                              statusMutation.mutate({ id: selectedDraft.id, status: "draft" })
                            }
                            className="px-3 py-1.5 text-xs font-semibold bg-gray-100 text-gray-700 hover:bg-gray-200 rounded-lg flex items-center gap-1"
                          >
                            <RefreshCw className="w-3 h-3" /> Revert
                          </button>
                          <button
                            onClick={() =>
                              statusMutation.mutate({ id: selectedDraft.id, status: "published" })
                            }
                            className="px-3 py-1.5 text-xs font-semibold bg-forest text-white hover:bg-forest rounded-lg flex items-center gap-1"
                          >
                            <Send className="w-3 h-3" /> Publish Live
                          </button>
                        </>
                      )}
                      {(selectedDraft.status === "published" ||
                        ...
                        <button
                          onClick={() =>
                            statusMutation.mutate({ id: selectedDraft.id, status: "archived" })
                          }
                          className="px-3 py-1.5 text-xs font-semibold bg-slate-100 text-slate-700 hover:bg-slate-200 rounded-lg flex items-center gap-1"
                        >
                  ...
                  <div className="space-y-4 max-w-2xl">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full p-2 text-sm border rounded-lg focus:ring-2 focus:ring-forest/20 focus:border-forest"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-gray-700 mb-1">
                      ...
                      <div className="flex items-center">
                        <span className="text-sm text-gray-400 bg-gray-50 border border-r-0 rounded-l-lg px-3 py-2">
                          /
                        </span>
                        <input
                          type="text"
                          ...
                          className="w-full p-2 text-sm border rounded-r-lg focus:ring-2 focus:ring-forest/20 focus:border-forest"
                        />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setEditedData({ ...editedData, meta_title: e.target.value })
                          }
                          className="w-full p-2 text-sm border rounded-lg focus:ring-2 focus:ring-forest/20 focus:border-forest"
                          rows={2}
                        />
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setEditedData({ ...editedData, meta_description: e.target.value })
                          }
                          className="w-full p-2 text-sm border rounded-lg focus:ring-2 focus:ring-forest/20 focus:border-forest"
                          rows={2}
                        />
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full p-3 font-mono text-xs border rounded-lg focus:ring-2 focus:ring-forest/20 focus:border-forest bg-gray-50"
                        rows={15}
                      />
                    </div>
                    <div className="pt-4 flex justify-end gap-2 border-t">
                      ...
                      <button
                        onClick={() => saveMutation.mutate(editedData)}
                        disabled={saveMutation.isPending}
                        className="px-4 py-2 text-sm font-medium bg-forest text-white hover:bg-forest rounded-lg"
                      >
                        Save Changes
                      </button>
                    </div>
                  ...
                  <div className="prose prose-sm prose-emerald max-w-none">
                    <div className="mb-8 p-4 bg-gray-50 rounded-xl border border-gray-100 space-y-2">
                      <p className="text-xs text-gray-500">
                        <strong>URL:</strong> /{selectedDraft.slug}
                      </p>
                      <p className="text-xs text-gray-500">
                        <strong>Meta Title:</strong> {selectedDraft.meta_title}
                      </p>
                      <p className="text-xs text-gray-500">
                        <strong>Meta Desc:</strong> {selectedDraft.meta_description}
                      </p>
                    </div>
