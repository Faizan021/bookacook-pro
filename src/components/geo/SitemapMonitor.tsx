...
function LogDot({ level }: { level: LogLevel }) {
  ...
  return (
    <span className={`inline-block w-2 h-2 rounded-full flex-shrink-0 mt-[5px] ${colors[level]}`} />
  );
}
...
export function SitemapMonitor() {
  ...
  const unindexedPages = allDrafts.filter(
    (d: any) => d.status === "published" && !d.sitemap_indexed,
  );
  ...
  async function handleCheckSitemap() {
    ...
    try {
      ...
      addLog("Sitemap erreichbar (CORS eingeschränkt) — Vercel Deployment aktiv", "warning");
          ...
          <div className="relative h-3 rounded-full bg-forest/10 overflow-hidden">
            ...
            <div
              ...
              style={{
                ...
                background: "linear-gradient(90deg, #133A25 0%, #1a5c38 50%, #133A25 100%)",
          ...
          {unindexedPages.length === 0 ? (
            <div className="p-8 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-sm font-medium text-forest/50">
                All published pages are verified.
              </p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-100">
                    <span className="text-xs text-gray-500 font-medium">
                      Published:{" "}
                      {new Date(page.published_at || page.updated_at).toLocaleDateString()}
                    </span>
                    <button
