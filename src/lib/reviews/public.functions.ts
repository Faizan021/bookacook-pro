...
export const getPublicRestaurantReviews = createServerFn({ method: "GET" })
  .validator((input: { restaurantId: string }) => z.object({ restaurantId: z.string() }).parse(input))
  .handler(async ({ data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");

    const { data: reviews, error } = await supabaseAdmin
      .from("restaurant_reviews")
      .select("id, overall_rating, food_quality_rating, speed_rating, comment, vendor_reply, created_at, customer_name")
    ...
    const avgOverall = count > 0 ? (reviews as any[]).reduce((acc, r) => acc + r.overall_rating, 0) / count : 0;
    const avgFood = count > 0 ? (reviews as any[]).reduce((acc, r) => acc + (r.food_quality_rating || 0), 0) / count : 0;
    const avgSpeed = count > 0 ? (reviews as any[]).reduce((acc, r) => acc + (r.speed_rating || 0), 0) / count : 0;
...
export const getPublicCatererReviews = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const avgOverall = count > 0 ? (reviews as any[]).reduce((acc, r) => acc + r.overall_rating, 0) / count : 0;
    const avgFood = count > 0 ? (reviews as any[]).reduce((acc, r) => acc + (r.food_rating || 0), 0) / count : 0;
    const avgRel = count > 0 ? (reviews as any[]).reduce((acc, r) => acc + (r.reliability_rating || 0), 0) / count : 0;
    const avgComms = count > 0 ? (reviews as any[]).reduce((acc, r) => acc + (r.communication_rating || 0), 0) / count : 0;
    const avgVal = count > 0 ? (reviews as any[]).reduce((acc, r) => acc + (r.value_rating || 0), 0) / count : 0;
...
export const getPublicPlannerReviews = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const count = formattedReviews.length;
    const avgOverall = count > 0 ? (reviews as any[]).reduce((acc, r) => acc + r.overall_rating, 0) / count : 0;
    const avgCre = count > 0 ? (reviews as any[]).reduce((acc, r) => acc + (r.creativity_rating || 0), 0) / count : 0;
    const avgExec = count > 0 ? (reviews as any[]).reduce((acc, r) => acc + (r.execution_rating || 0), 0) / count : 0;
    const avgComm = count > 0 ? (reviews as any[]).reduce((acc, r) => acc + (r.communication_rating || 0), 0) / count : 0;
    const avgVal = count > 0 ? (reviews as any[]).reduce((acc, r) => acc + (r.value_rating || 0), 0) / count : 0;
