...
async function resolveOwnedRestaurant(supabase: any, userId: string) {
  const { data } = await supabase.from("restaurants").select("*").eq("owner_id", userId).maybeSingle();
  return data;
}
...
async function resolveOwnedPlanner(supabase: any, userId: string) {
  const { data } = await supabase.from("event_managers").select("*").eq("owner_id", userId).maybeSingle();
  return data;
}
...
export const getVendorRestaurantReviews = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabase
      ...
      .select("id, overall_rating, food_quality_rating, speed_rating, comment, vendor_reply, status, created_at, customer_name")
...
export const submitRestaurantVendorReply = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (existing.vendor_reply) throw new Error("Review already has a vendor reply. Single reply enforced.");
...
export const flagRestaurantReview = createServerFn({ method: "POST" })
  .middleware([requireRole("restaurant_owner")])
  .validator((input: { reviewId: string }) => z.object({ reviewId: z.string().uuid() }).parse(input))
  .handler(async ({ context, data }) => {
    const { supabase, userId } = context;
    const vendor = await resolveOwnedRestaurant(supabase, userId);
    if (!vendor) throw new Error("Not authorized");
