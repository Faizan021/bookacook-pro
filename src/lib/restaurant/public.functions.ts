...
export const getRestaurantBySlug = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: rest, error } = await supabaseAdmin
      .from("restaurants")
      .select(
        "id, owner_id, name, slug, custom_domain, stripe_connect_status, subscription_status, is_published, certifications",
      )
      .eq("slug", data.slug)
      .maybeSingle();
...
async function calculatePromoDiscount(
  ...
  if (promo.min_order_value_cents && subtotalCents < promo.min_order_value_cents) {
    return { discountCents: 0, freeDelivery: false, error: `Minimum order value: €${(promo.min_order_value_cents / 100).toFixed(2)}` };
  }
...
export const validatePromoCode = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const result = await calculatePromoDiscount(supabaseAdmin, rest.owner_id, data.promoCode, subtotalCents, validatedItems);
    if (result.error) {
...
export const submitStorefrontOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: rest, error: restErr } = await supabaseAdmin
      .from("restaurants")
      .select("id, owner_id, name, fee, paypal_email")
      .eq("id", data.restaurantId)
      .single();
    ...
    if (prodErr || !products || products.length === 0) throw new Error("Products not found or invalid");
    ...
    if (data.orderType === "delivery" && (rest as any).fee) {
      const parsedFee = parseFloat((rest as any).fee.replace(/[^\d.,]/g, "").replace(",", "."));
      if (!isNaN(parsedFee)) {
        deliveryFeeCents = Math.round(parsedFee * 100);
      }
    }
      ...
      if (data.paymentMethod === "paypal") {
        ...
        if (rawPaypal.startsWith("http")) {
          if (!rawPaypal.startsWith("https://paypal.me/")) {
            throw new Error("Invalid PayPal URL configured by merchant. Only paypal.me links are allowed.");
          }
