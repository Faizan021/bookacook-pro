...
import { Search, Star, Clock, MapPin, ChevronDown, Navigation, UtensilsCrossed, X, Zap, ShieldCheck, MapPin as MapPinIcon, ChevronRight, BadgeCheck } from "lucide-react";
...
export const Route = createFileRoute("/instant-order")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Restaurants in deiner Nähe mit Live-Verfügbarkeit, transparenten Preisen und schnellen Lieferzeiten." },
      { property: "og:title", content: "Sofort bestellen — Speisely" },
      { property: "og:description", content: "Live-Verfügbarkeit und transparente Preise bei Restaurants in deiner Nähe." },
...
function InstantOrder() {
  ...
  const suggestedCities = ["Berlin", "Hamburg", "München", "Köln", "Frankfurt", "Stuttgart", "Düsseldorf", "Leipzig"];
  ...
  const filtered = useMemo(() => {
    ...
    let list = restaurants.filter((r) => {
      if (lowerQ && !(`${r.name} ${r.tags.join(" ")} ${r.area}`.toLowerCase().includes(lowerQ))) return false;
      ...
      switch (activeFilter) {
        ...
        case "asian":
          return tagSet.some((x) => ["japanese", "asian", "chinese", "thai", "vietnamese"].includes(x));
        case "vegan":
  ...
  const instantOrderFaqs = [
    {
      question: { de: "Was ist der Unterschied zwischen Speisely und anderen Lieferdiensten?", en: "What's the difference between Speisely and other delivery apps?" },
      answer: { de: "Bei Speisely bestellen Sie provisionsfrei. Die Restaurants zahlen 0% Gebühren an uns, wodurch sie Ihnen oft bessere Preise oder größere Portionen anbieten können.", en: "With Speisely, you order commission-free. Restaurants pay 0% fees to us, which often allows them to offer you better prices or larger portions." }
    },
    {
      question: { de: "Wie lange dauert die Lieferung im Durchschnitt?", en: "How long does delivery take on average?" },
      answer: { de: "Die Lieferzeit hängt vom gewählten Restaurant und Ihrer Entfernung ab. Die meisten Bestellungen werden innerhalb von 30 bis 45 Minuten geliefert.", en: "The delivery time depends on the chosen restaurant and your distance. Most orders are delivered within 30 to 45 minutes." }
    },
    {
      question: { de: "Gibt es einen Mindestbestellwert für Sofortbestellungen?", en: "Is there a minimum order value for instant orders?" },
      answer: { de: "Jedes Restaurant legt seinen eigenen Mindestbestellwert fest. Sie sehen diesen direkt in der Übersichtskarte des jeweiligen Restaurants.", en: "Each restaurant sets its own minimum order value. You can see this directly in the overview card of the respective restaurant." }
    }
  ];
    ...
    <SiteShell>
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-10 grid md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-display font-bold text-white leading-[1.05] tracking-tight drop-shadow-sm">
              ...
              <span className="text-[#b28a3c]">{tt("100% provisionsfrei.", "100% commission-free.")}</span>
            </h1>
          ...
          <div className="relative rounded-[2rem] overflow-hidden shadow-2xl border border-white/10 aspect-[4/3] max-h-[360px] group">
             <img src="https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=1200&q=80" alt="Fast restaurant delivery" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 ease-out" />
             ...
             <div className="absolute bottom-6 left-6 bg-white/10 backdrop-blur-md rounded-xl p-3 shadow-xl border border-white/20 flex items-center gap-3">
                ...
                <div className="text-[12px] font-bold text-white leading-none">
                  ...
                  <div className="text-[10px] text-white/60 font-medium mt-1">{tt("Keine versteckten Gebühren", "No hidden fees")}</div>
                </div>
              </div>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10 pt-10" id="listings">
        <div className="relative inline-block">
          <button
            type="button"
            onClick={() => { setLocDraft(location); setLocOpen((o) => !o); }}
          ...
          >
            ...
            {location ? t("io.deliveryToSelected").replace("{location}", location) : t("io.deliveryToPlaceholder")}
          ...
          {locOpen && (
            <>
              ...
              <div className="absolute left-0 top-full mt-2 z-40 w-[320px] rounded-2xl bg-white shadow-xl ring-1 ring-forest/10 p-4">
                <label htmlFor="delivery-address" className="block text-[11px] font-semibold uppercase tracking-[0.16em] text-forest/60">
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && locDraft.trim()) { setLocation(locDraft.trim()); setLocOpen(false); }
                  }}
                ...
                <button
                  type="button"
                  onClick={() => { setLocation(tt("Aktueller Standort", "Current location")); setLocOpen(false); }}
                  className="mt-3 inline-flex items-center gap-2 text-sm text-forest hover:text-[#b28a3c]"
                >
                  <Navigation className="h-4 w-4" /> {tt("Aktuellen Standort verwenden", "Use current location")}
                </button>
                <div className="mt-4">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-forest/50">{t("io.popularCities")}</p>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {suggestedCities.map((city) => (
                      <button
                        key={city}
                        type="button"
                        onClick={() => { setLocation(city); setLocOpen(false); }}
                        className={`rounded-full px-3 py-1.5 text-sm transition ${
                          location === city ? "bg-[#b28a3c] text-[#16372f] font-semibold" : "bg-cream text-forest hover:bg-[#eadfce]"
                        }`}
                      >
                        {city}
                      </button>
                ...
                <div className="mt-4 flex justify-end gap-2">
                  <button onClick={() => setLocOpen(false)} className="rounded-full px-4 py-2 text-sm text-forest/70 hover:text-forest">
                    {tt("Abbrechen", "Cancel")}
                  </button>
                  <button
                    onClick={() => { if (locDraft.trim()) { setLocation(locDraft.trim()); setLocOpen(false); } }}
        ...
        <div className="mt-3 grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <h2 className="text-2xl sm:text-3xl font-display font-bold text-forest leading-[0.95]">{t("io.nearYou")}</h2>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-2">
          {filters.map((f) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm transition ${
                activeFilter === f.id ? "bg-forest text-[oklch(0.97_0.02_92)]" : "bg-cream text-forest hover:bg-[oklch(0.93_0.04_92)]"
              }`}
            >
              {f.label}
            </button>
          ...
          <div className="ml-auto inline-flex items-center gap-2 text-sm text-forest/70">
            <label htmlFor="sortby" className="whitespace-nowrap">{tt("Sortieren nach", "Sort by")}</label>
        ...
        {filtered.length === 0 ? (
          <div className="mx-auto max-w-md text-center py-16">
            ...
            <p className="mt-2 text-sm text-forest/70">
              {tt("Versuche eine andere Suche oder passe deine Filter an.", "Try a different search or adjust your filters.")}
            </p>
            <button
              ...
              >
                ...
                <div className="mt-4 px-1 pb-1">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <h3 className="font-display text-lg font-bold text-forest truncate">{r.name}</h3>
                  </div>
                  ...
                  <div className="flex items-center gap-3 text-xs font-semibold text-forest/70">
                    <span className="inline-flex items-center gap-1"><MapPin className="h-3.5 w-3.5" /> {r.distanceKm.toFixed(1)} km</span>
                    <span className="text-forest/30">•</span>
                    <span>{t("io.delivery")} {r.fee}</span>
                    <span className="text-forest/30">•</span>
                    <span>{tt("Mind.", "Min.")} €{r.minOrder}</span>
                  </div>
                </div>
              </Link>
            ))}
      ...
      <section className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-10 py-16">
        <div className="text-center mb-10">
          <h2 className="text-2xl sm:text-3xl font-display font-bold text-forest mb-4">
            {tt("Häufig gestellte Fragen zur Sofortbestellung", "Frequently Asked Questions about Instant Orders")}
          </h2>
        </div>
        <div className="space-y-6">
