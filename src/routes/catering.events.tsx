...
function EventsCatering() {
  ...
  return (
    <SiteShell>
      <PageHero
        eyebrow={
          lang === "de" ? "EINMALIGE EVENTS & FEIERLICHKEITEN" : "ONE-OFF EVENTS & CELEBRATIONS"
        }
        heading={
          lang === "de" ? (
            <>
              ...
              <br />
              mit <span className="text-[#f2d896]">Premium Event-Catering.</span>
            </>
          ) : (
            <>
              Unforgettable moments
              <br />
              with <span className="text-[#f2d896]">premium event catering.</span>
            </>
          )
        }
        subtext={
        ...
        primaryCta={{
          label: lang === "de" ? "Event-Caterer finden" : "Discover Event Caterers",
          href: "/catering",
        }}
        secondaryCta={{
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10 py-16 md:py-24 border-b border-[#eadfce]/30">
        <div className="text-center max-w-2xl mx-auto space-y-4">
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-forest">
            {lang === "de" ? "UNSERE FORMATS" : "OUR FORMATS"}
          </span>
          <h2 className="text-3xl sm:text-4xl font-display font-bold text-forest">
            {lang === "de"
              ? "Kreative Genusskonzepte für dein Event"
              : "Creative Culinary Concepts for Your Event"}
          </h2>
        ...
        <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="surface-card p-8 rounded-3xl border border-[#eadfce]/45 flex flex-col justify-between text-left">
            <div className="space-y-4">
              ...
              <h3 className="text-xl font-display font-bold text-forest">
                {lang === "de"
                  ? "Privat-Dinner & Business Lunch"
                  : "Private Dinner & Business Lunch"}
              </h3>
              <p className="text-sm text-forest/75 leading-relaxed">
                {lang === "de"
        ...
        <div className="rounded-[2.5rem] bg-forest p-8 sm:p-12 text-cream flex flex-col md:grid md:grid-cols-[1.2fr_0.8fr] items-center gap-12 relative overflow-hidden shadow-xl border border-forest/10">
          <div className="relative z-10 space-y-5 text-left">
            <span className="inline-flex items-center gap-2 rounded-full bg-cream/20 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-cream backdrop-blur-sm">
              ...
              {lang === "de" ? "KULINARISCHE SPITZENKLASSE" : "CULINARY EXCELLENCE"}
            </span>
            <h2 className="font-display text-3xl sm:text-4xl text-mint leading-tight">
              {lang === "de"
                ? "Vergleiche die besten Angebote live"
                : "Compare the Best Quotes Live"}
            </h2>
      ...
      <section className="bg-cream/10 py-16 md:py-24 border-t border-b border-[#eadfce]/30 text-left">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
          <div className="max-w-2xl space-y-3">
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-forest">
              {lang === "de" ? "ABLAUF" : "HOW IT WORKS"}
            </span>
            <h2 className="text-3xl font-display font-bold text-forest">
              {lang === "de"
                ? "In 3 Schritten zum perfekten Event-Menü"
                : "In 3 Steps to Your Perfect Event Menu"}
            </h2>
          </div>
          <div className="mt-12 grid gap-8 sm:grid-cols-3">
            <div className="space-y-3">
              ...
              <h4 className="font-bold text-forest text-base">
                {lang === "de" ? "Caterer auswählen" : "Select Caterer"}
              </h4>
              <p className="text-xs text-forest/70 leading-relaxed">
                {lang === "de"
                  ? "Filtere nach Anlass, Personenanzahl und Ort, und entdecke unsere geprüften Partner."
                  : "Filter by occasion, guest count, and location, and explore our verified catering partners."}
              </p>
            </div>
            <div className="space-y-3">
              <div className="text-4xl font-display font-bold text-forest">02</div>
              <h4 className="font-bold text-forest text-base">
                {lang === "de" ? "Konzept anfordern" : "Request Proposal"}
              </h4>
              <p className="text-xs text-forest/70 leading-relaxed">
                {lang === "de"
                  ? "Wähle ein vordefiniertes Menü oder frage ein individuelles Konzept an. Kostenlos & unverbindlich."
                  : "Choose a predefined package or request a custom menu path. Free of charge & non-binding."}
              </p>
            </div>
            <div className="space-y-3">
              <div className="text-4xl font-display font-bold text-forest">03</div>
              <h4 className="font-bold text-forest text-base">
                {lang === "de" ? "Genießen" : "Celebrate & Enjoy"}
              </h4>
              <p className="text-xs text-forest/70 leading-relaxed">
                {lang === "de"
                  ? "Besprich letzte Details direkt mit dem Caterer. Pünktliche Lieferung und Aufbau vor Ort sind garantiert."
                  : "Clarify details directly with the food partner. Logistical accuracy and on-site setup are fully guaranteed."}
              </p>
            </div>
          </div>
        </div>
