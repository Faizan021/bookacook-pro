              ...
              <div className="mt-8 flex items-center justify-between border-t border-forest/10 pt-6">
                ...
                <button 
                  onClick={() => {
                    ...
                    toast.success(tt("Link in die Zwischenablage kopiert!", "Link copied to clipboard!"));
                  }}
            ...
            <div className="mt-16 rounded-2xl bg-cream border border-[#eadfce] p-8 sm:p-10 text-center shadow-sm">
              <h3 className="font-display text-3xl text-forest mb-4">
                {post.category === "Catering" 
                  ? tt("Bereit für dein nächstes Event-Catering?", "Ready for your next event catering?")
              ...
              <p className="text-forest/75 mb-8 max-w-lg mx-auto">
                {post.category === "Catering"
                  ? tt("Entdecke handverlesene Caterer auf unserem Marktplatz.", "Discover hand-picked caterers on our marketplace.")
                  : post.category === "Event Planning"
                  ? tt("Finde geprüfte Eventplaner für dein nächstes großes Projekt.", "Find vetted event planners for your next big project.")
                  : post.category === "Company"
                  ? tt("Registriere dein Unternehmen als Partner und profitiere von 0% Bestellprovision.", "Register your business as a partner and benefit from 0% order commission.")
                  : tt("Unterstütze lokale Restaurants und bestelle direkt über Speisely.", "Support local restaurants and order directly via Speisely.")}
              </p>
              <Link
                to={post.category === "Catering" ? "/catering" : post.category === "Event Planning" ? "/planner" : post.category === "Company" ? "/partners" : "/instant-order"}
                onClick={() => trackEvent("related_service_cta_clicked", { location: "blog_detail_footer", category: post.category })}
