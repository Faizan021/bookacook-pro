...
export const Route = createFileRoute("/restaurants")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Entdecke unabhängige Restaurants auf dem Speisely Marketplace und bestelle direkt. Fair für die Restaurants, besser für dich." },
      { property: "og:title", content: "Restaurants Marketplace | Speisely" },
      { property: "og:description", content: "Entdecke unabhängige Restaurants auf dem Speisely Marketplace und bestelle direkt." },
...
function RestaurantsDirectory() {
  ...
  const getRestaurantUrl = (rest: any) => {
    if (rest.custom_domain) {
      const domain = rest.custom_domain.startsWith("http")
        ? rest.custom_domain
        : `https://${rest.custom_domain}`;
      return `${domain}?ref=speisely_marketplace`;
    }
          ...
          <div className="max-w-7xl mx-auto px-6 py-16 w-full">
            {restaurants.length === 0 ? (
              <div className="text-center py-24 bg-white rounded-3xl shadow-sm border border-[#e2e8e4]">
                ...
                <h3 className="text-2xl font-display text-forest mb-2">No restaurants available yet</h3>
                <p className="text-gray-500">Check back soon as more local partners join the marketplace.</p>
              </div>
            ) : (
                      ...
                      <div className="absolute -bottom-8 left-6 z-20">
                        {rest.logo_url ? (
                          <div className="w-16 h-16 rounded-full border-4 border-white shadow-md bg-white overflow-hidden">
                            <img src={rest.logo_url} alt={`${rest.name} Logo`} className="w-full h-full object-cover" />
                          </div>
                        ) : (
