...
import {
  getVendorPlannerReviews,
  submitPlannerVendorReply,
  flagPlannerReview,
...
import { Star, MessageSquare, AlertTriangle, CheckCircle, Clock, Tag } from "lucide-react";
import { useI18n } from "@/i18n/I18nProvider";
import { toast } from "sonner";

const reviewsQueryOptions = queryOptions({
  queryKey: ["vendorPlannerReviews"],
  queryFn: () => getVendorPlannerReviews(),
});

export const Route = createFileRoute("/_authenticated/dashboard/planner/reviews")({
  beforeLoad: async () => {
  ...
  component: PlannerReviewsDashboard,
});
...
function ReviewCard({ review }: { review: any }) {
  ...
  const submitReply = useServerFn(submitPlannerVendorReply);
  const flagReview = useServerFn(flagPlannerReview);
  ...
  const replyMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["vendorPlannerReviews"] });
    },
      ...
      <div className="flex justify-between items-start">
        <div>
          <h4 className="font-display font-semibold text-forest text-lg">
            {review.customer_name || t("Kunde", "Customer")}
          </h4>
          <div className="flex items-center gap-2 mt-1">
            ...
            {review.event_type && (
              <span className="inline-flex items-center gap-1 text-[10px] uppercase font-bold tracking-wider text-forest/50 bg-forest/5 px-2 py-0.5 rounded-sm">
...
function PlannerReviewsDashboard() {
  const { lang } = useI18n();
  const t = (de: string, en: string) => (lang === "de" ? de : en);
  ...
  return (
    <VendorLayout title={t("Bewertungen", "Reviews")} vertical="planner">
      <div className="max-w-4xl space-y-10">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-display font-bold text-forest">
            {t("Kundenbewertungen", "Customer Reviews")}
          </h2>
          <Link to="/dashboard/planner" className="text-sm font-semibold text-forest underline">
