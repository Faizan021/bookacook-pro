...
const reviewsQueryOptions = queryOptions({
  queryKey: ["vendorCatererReviews"],
  queryFn: () => getVendorCatererReviews(),
});

export const Route = createFileRoute("/_authenticated/caterer/reviews")({
  beforeLoad: async () => {
    const profile = await getUserProfile();
    if (!profile.roles.includes("caterer")) {
  ...
  component: CatererReviewsDashboard,
});
...
function ReviewCard({ review }: { review: any }) {
  ...
  const submitReply = useServerFn(submitCatererVendorReply);
  const flagReview = useServerFn(flagCatererReview);
  ...
  const replyMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["vendorCatererReviews"] });
    },
...
function CatererReviewsDashboard() {
  const { lang } = useI18n();
  const t = (de: string, en: string) => (lang === "de" ? de : en);
  ...
  return (
    <VendorLayout vertical="caterer" title={t("Bewertungen", "Reviews")}>
      <div className="max-w-4xl space-y-10">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-display font-bold text-forest">
            {t("Kundenbewertungen", "Customer Reviews")}
          </h2>
          <Link to="/caterer" className="text-sm font-semibold text-forest underline">
