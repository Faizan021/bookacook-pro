      ...
      <div className="min-h-screen bg-mint-dotted flex items-center justify-center px-4 py-16">
        <div className="surface-card w-full max-w-md p-8 text-center space-y-4">
          <h1 className="text-3xl font-display text-forest">
            {tt("Link ungültig oder abgelaufen", "Link invalid or expired")}
          </h1>
          <p className="text-forest/70">
            {tt(
              "Bitte fordere einen neuen Link zum Zurücksetzen des Passworts an.",
    ...
    <div className="min-h-screen bg-mint-dotted flex items-center justify-center px-4 py-16">
      <div className="surface-card w-full max-w-md p-8">
        ...
        <h1 className="mt-4 text-3xl font-display text-forest">
          {tt("Neues Passwort vergeben", "Set new password")}
        </h1>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="password" className="text-forest">
              {tt("Neues Passwort", "New Password")} <span className="text-red-500">*</span>
            </Label>
          ...
          <Button
            type="submit"
            className="w-full bg-[#b28a3c] hover:opacity-90 text-[#16372f] font-semibold mt-6"
            disabled={loading}
          >
            {loading
              ? tt("Bitte warten…", "Please wait…")
              : tt("Passwort speichern", "Save password")}
          </Button>
        </form>
      </div>
    </div>
