...
function AuthPage() {
  ...
  const [mode, setMode] = useState<"signin" | "signup" | "check-email" | "forgot-password">(initialMode);
  ...
  const formSchema = z.object({
    ...
    password: mode === "signup" ? passwordSchema : z.string().min(1, tt("Passwort erforderlich.", "Password required.")),
    confirmPassword: z.string().optional(),
    fullName: mode === "signup" ? z.string().min(2, tt("Name erforderlich.", "Name required.")) : z.string().optional(),
    ...
    if (mode === "signup") {
      ...
      if (!data.termsAccepted) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: tt("Bitte akzeptieren Sie die Nutzungsbedingungen.", "Please accept the Terms of Service."),
          path: ["termsAccepted"],
        });
      }
      if (isPartner) {
        if (!data.business_name) ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Erforderlich", path: ["business_name"] });
        if (!data.contact_person) ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Erforderlich", path: ["contact_person"] });
        if (!data.business_address) ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Erforderlich", path: ["business_address"] });
      }
    }
  });
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      email: "", password: "", confirmPassword: "", fullName: "", termsAccepted: false, marketingOptIn: false,
      business_name: "", contact_person: "", phone: "", license_number: "", business_address: "", service_city: "", service_area: ""
    }
  });
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      if (mode === "forgot-password") {
        ...
        setGlobalSuccess(tt("Wenn diese E-Mail existiert, haben wir einen Link zum Zurücksetzen gesendet.", "If this email exists, we have sent a reset link."));
        return;
      }
  ...
  const handleResendEmail = async () => {
    ...
    try {
      ...
      if (error) throw error;
      setGlobalSuccess(tt("Bestätigungs-E-Mail wurde erneut gesendet.", "Verification email resent."));
    } catch (e: any) {
      setGlobalErr(sanitizeAuthError(e, lang));
    } finally {
      setLoading(false);
  ...
  function sanitizeAuthError(e: any, lang: string): string {
    ...
    if (code === "profile_does_not_exist") return de("Ein Profil mit dieser E-Mail-Adresse existiert nicht. Bitte registrieren Sie sich zuerst.") ?? "No profile exists with this email. Please sign up first.";
    if (code.startsWith("incorrect_password_with_role:")) return de("Falsches Passwort für dieses Konto.") ?? "Incorrect password for this account.";
    if (code.includes("invalid_credentials") || code.includes("Invalid login credentials")) return de("E-Mail oder Passwort ist falsch.") ?? "Incorrect email or password.";
    if (code.includes("email_not_confirmed")) return de("Bitte bestätige zuerst deine E-Mail-Adresse.") ?? "Please confirm your email address first.";
    if (code.includes("user_already_exists") || code.includes("already registered")) return de("Diese E-Mail-Adresse ist bereits registriert.") ?? "An account with this email already exists.";
    if (code.includes("over_email_send_rate_limit") || code.includes("rate limit")) return de("Zu viele Anfragen. Bitte warte einen Moment.") ?? "Too many requests. Please wait a moment.";
      ...
      <div className="min-h-screen bg-mint-dotted flex items-center justify-center px-4 py-16">
        <div className="surface-card w-full max-w-md p-8 text-center space-y-4">
          ...
          <p className="text-forest/70">
            {tt("Wir haben dir einen Bestätigungslink an", "We sent a confirmation link to")} <strong>{currentEmail}</strong> {tt("gesendet.", "sent.")}
          </p>
          ...
          <Button variant="outline" onClick={handleResendEmail} disabled={loading} className="w-full mt-4">
            {loading ? tt("Wird gesendet...", "Sending...") : tt("E-Mail erneut senden", "Resend Email")}
          </Button>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-mint-dotted flex items-center justify-center px-4 py-16">
      <div className={`surface-card w-full ${mode === "signup" && isPartner ? "max-w-2xl" : "max-w-md"} p-8`}>
        ...
        <h1 className="mt-4 text-3xl font-display text-forest">
          {mode === "signup" ? tt("Konto erstellen", "Create an account") : 
           mode === "forgot-password" ? tt("Passwort zurücksetzen", "Reset Password") : 
           tt("Willkommen zurück", "Welcome back")}
        </h1>
        <p className="mt-1 text-sm text-forest/70 mb-6">
          {mode === "signup" ? tt("Bestelle Essen oder verwalte dein Geschäft.", "Start ordering or manage your storefront.") : 
           mode === "forgot-password" ? tt("Gib deine E-Mail ein, um dein Passwort zurückzusetzen.", "Enter your email to reset your password.") :
           tt("Melde dich in deinem Dashboard an.", "Sign in to your dashboard.")}
        </p>

        {message && <div className="mb-4 p-3 rounded-md bg-amber-50 border border-amber-200 text-sm text-amber-700">{message}</div>}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {mode !== "forgot-password" && (
            <div className="space-y-1.5">
              <Label htmlFor="password" className="text-forest">{tt("Passwort", "Password")} <span className="text-red-500">*</span></Label>
          ...
          {mode === "signup" && isPartner && (
            <div className="rounded-xl border border-forest/20 bg-forest/5 p-4 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1"><Label className="text-xs">{tt("Unternehmensname", "Business name")}*</Label><Input {...register("business_name")} className={errors.business_name ? "border-red-500" : "border-[#eadfce]"} /></div>
                <div className="space-y-1"><Label className="text-xs">{tt("Ansprechpartner", "Contact person")}*</Label><Input {...register("contact_person")} className={errors.contact_person ? "border-red-500" : "border-[#eadfce]"} /></div>
                <div className="space-y-1"><Label className="text-xs">{tt("Telefonnummer", "Phone number")}</Label><Input type="tel" {...register("phone")} className="border-[#eadfce]" /></div>
                <div className="space-y-1"><Label className="text-xs">{tt("Gewerbeanmeldung / HRB", "License / Registration #")}</Label><Input {...register("license_number")} className="border-[#eadfce]" /></div>
                <div className="sm:col-span-2 space-y-1"><Label className="text-xs">{tt("Geschäftsadresse", "Business address")}*</Label><Input {...register("business_address")} className={errors.business_address ? "border-red-500" : "border-[#eadfce]"} /></div>
                <div className="space-y-1"><Label className="text-xs">{tt("Stadt", "Service city")}</Label><Input {...register("service_city")} className="border-[#eadfce]" /></div>
                <div className="space-y-1"><Label className="text-xs">{tt("Stadtteil", "Service area")}</Label><Input {...register("service_area")} className="border-[#eadfce]" /></div>
              </div>
            </div>
          )}
          ...
          {mode === "signup" && (
            <div className="space-y-3 py-2 border-t border-[#eadfce]/60 mt-4 pt-4">
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" {...register("termsAccepted")} className="mt-1 h-4 w-4 rounded border-[#eadfce] text-[#b28a3c] focus:ring-[#b28a3c]" />
                <span className="text-sm text-forest/80">
                  {tt("Ich akzeptiere die Nutzungsbedingungen.", "I agree to the Terms of Service.")} <span className="text-red-500">*</span>
                </span>
              </label>
              {errors.termsAccepted && <p className="text-xs text-red-500 ml-7">{errors.termsAccepted.message}</p>}
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" {...register("marketingOptIn")} className="mt-1 h-4 w-4 rounded border-[#eadfce] text-[#b28a3c] focus:ring-[#b28a3c]" />
                <span className="text-sm text-forest/80">
          ...
          <Button type="submit" className="w-full bg-[#b28a3c] hover:opacity-90 text-[#16372f] font-semibold" disabled={loading}>
            {loading ? tt("Bitte warten…", "Please wait…") : 
             mode === "signup" ? tt("Konto erstellen", "Create account") : 
             mode === "forgot-password" ? tt("Link senden", "Send Reset Link") :
             tt("Anmelden", "Sign in")}
          </Button>
        </form>
        ...
        {mode !== "forgot-password" && (
          <>
            <div className="relative my-6">
              <div className="absolute inset-0 flex items-center"><span className="w-full border-t border-[#eadfce]" /></div>
              <div className="relative flex justify-center text-xs uppercase"><span className="bg-[#fcf8f2] px-2 text-forest/50">Oder</span></div>
            </div>
