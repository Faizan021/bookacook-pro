...
function FaqPage() {
  ...
  const toggleCatering = (index: number) => setOpenCateringIndex(openCateringIndex === index ? null : index);
  const togglePlanner = (index: number) => setOpenPlannerIndex(openPlannerIndex === index ? null : index);

  const curCateringFaq = cateringFaqData[lang as keyof typeof cateringFaqData] || cateringFaqData.en;
