const fs = require("fs");
let code = fs.readFileSync("src/routes/_authenticated/restaurant.tsx", "utf8");
...
code = code.replace('<div className="pt-2 pb-1', selectHtml);
...
code = code.replace(
  /<PromotionsSection vertical="restaurants" \/>/,
  '<PromotionsSection vertical="restaurants" availableItems={(q.data.restaurant?.restaurant_products || []).map((p: any) => p.name)} />',
);

fs.writeFileSync("src/routes/_authenticated/restaurant.tsx", code);
console.log("restaurant.tsx patched");
