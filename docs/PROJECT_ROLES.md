...
**Permissions & Restrictions:** Full read/write access to admin tables. **Important clarification:** admin's publish-toggle capability is an *override*, not a *gate* — restaurants and caterers can self-publish by default; admin has no built-in pre-publish approval step. Do not assume moderation-before-launch exists.
...
*Note: The unified `partner` role is injected by middleware for any user possessing `caterer`, `restaurant_owner`, or `planner`. Confirmed exactly as described in `role-middleware.ts`.*
