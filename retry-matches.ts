...
async function main() {
  const rows = JSON.parse(
    fs.readFileSync("migration-audit/failed_event_request_matches.json", "utf-8"),
  );
  ...
  if (error) {
    ...
    fs.writeFileSync(
      "migration-audit/migrated_event_request_matches.json",
      JSON.stringify(fixed, null, 2),
    );
    fs.unlinkSync("migration-audit/failed_event_request_matches.json");
    console.log(
      "Audit file updated: failed_event_request_matches.json -> migrated_event_request_matches.json",
    );
  }
}
main().catch((e) => {
