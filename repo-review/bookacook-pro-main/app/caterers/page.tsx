...
import { getMarketplaceCaterers, type MarketplaceCaterer } from "@/lib/marketplace/queries";

export default function CaterersPage() {
  const t = useT();

  const [caterers, setCaterers] = useState<MarketplaceCaterer[]>([]);
  ...
  useEffect(() => {
    async function loadCaterers() {
      setLoading(true);
      const data = await getMarketplaceCaterers();
      setCaterers(data);
      setLoading(false);
    }
    loadCaterers();
  ...
  }, [caterers, search, selectedLocation, eventType, style]);
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24">
        ...
        <div className="mt-14">
          ...
          <h2 className="mt-3 text-4xl font-semibold tracking-tight">
            {loading ? "..." : filteredCaterers.length}{" "}
        ...
        {loading ? (
          <div className="mt-8 flex justify-center py-20">
            <div className="h-12 w-12 animate-spin rounded-full border-4 border-[#173f35] border-t-transparent"></div>
          </div>
