...
export function PackageForm({ mode, packageId, initialData }: PackageFormProps) {
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof PackageFormData | "global", string>>
  >({});
  ...
  function validate(forPublish: boolean): boolean {
    ...
    if (!form.price_amount || form.price_amount <= 0) {
      errs.price_amount = t("pkg.validation.priceRequired");
    }
    if (!form.min_guests || !form.max_guests) {
      errs.min_guests = t("pkg.validation.guestsRequired");
    }
    if ((form.min_guests ?? 0) > (form.max_guests ?? 0)) {
      errs.max_guests = t("pkg.validation.guestsOrder");
    }
    if (forPublish && !(form.summary ?? "").trim()) {
      errs.summary = t("pkg.validation.summaryRequired");
    }
    if (forPublish && !(form.description ?? "").trim()) {
      errs.description = t("pkg.validation.descRequired");
    }
  ...
  function applyAiSuggestion() {
    ...
    if (aiTarget === "title") {
      set("title", aiSuggestion);
    } else if (aiTarget === "description") {
      set("description", aiSuggestion);
    } else if (aiTarget === "tags") {
      const newTags = aiSuggestion
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
    ...
    <div className="mx-auto max-w-4xl space-y-4 pb-12">
      ...
      <Section title={t("pkg.form.basics")} badge={t("pkg.form.required")}>
        <div className="space-y-4">
          <div>
            ...
            <TipPanel
              tipTitle={t("pkg.tip.titleTitle")}
              tips={[t("pkg.tip.title1"), t("pkg.tip.title2"), t("pkg.tip.title3")]}
            />
            {aiTarget === "title" ? <AiResult /> : null}
          </div>
      ...
      <Section title={t("pkg.form.description")}>
        <div>
          ...
          <TipPanel
            tipTitle={t("pkg.tip.descTitle")}
            tips={[t("pkg.tip.desc1"), t("pkg.tip.desc2"), t("pkg.tip.desc3")]}
          />
          {aiTarget === "description" ? <AiResult /> : null}
        </div>
      </Section>
      ...
      <CollapsibleSection
        title={t("pkg.form.logistics")}
        open={showLogistics}
        onToggle={() => setShowLogistics((v) => !v)}
      >
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                set("setup_time_hours", parseFloat(e.target.value) || 0)
              }
              min={0}
              step={0.5}
              className={inputCls}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                set("booking_notice_days", parseInt(e.target.value) || 0)
              }
              min={0}
              className={inputCls}
            />
      ...
      >
        <div className="space-y-4">
          ...
          <TipPanel
            tipTitle={t("pkg.tip.imageTitle")}
            tips={[t("pkg.tip.image1"), t("pkg.tip.image2"), t("pkg.tip.image3")]}
          />
        </div>
      </CollapsibleSection>

      <CollapsibleSection
        title={t("pkg.form.discovery")}
        open={showDiscovery}
