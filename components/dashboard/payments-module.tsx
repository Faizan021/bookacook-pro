    ...
    <div className="space-y-6 text-[#16372f]">
      ...
      <div>
        ...
        <p className="mt-2 text-sm text-[#5c6f68]">
          Monitor platform revenue, commission, and payouts.
        </p>
      </div>
      ...
      {role === "admin" && adminSummary && (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          <SummaryCard label="GMV" value={adminSummary.gmv} />
          <SummaryCard label="Commission (10%)" value={adminSummary.commissionTotal} />
          <SummaryCard label="Held funds" value={adminSummary.heldFunds} />
          <SummaryCard label="Released" value={adminSummary.releasedTotal} />
          <SummaryCard label="Blocked payouts" value={adminSummary.blockedPayouts} />
        </div>
      )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[700px] text-sm">
              ...
              <tbody className="divide-y divide-[#eadfce]">
                {payments.map((p) => (
                  <tr key={p.id} className="hover:bg-[#faf6ee]">
                    <td className="px-6 py-4 text-xs text-[#8a6d35] font-mono">
                      {p.id}
                    </td>

                    <td className="px-6 py-4 text-[#173f35]">
                      {p.description}
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`rounded-full px-3 py-1 text-[11px] font-semibold uppercase ${STATUS_STYLE[p.status]}`}
                      >
                        {p.status}
                      </span>
                    </td>
...
function SummaryCard({
  ...
  return (
    <div className="rounded-[1.6rem] border border-[#eadfce] bg-white p-5 shadow-sm">
      <p className="text-sm text-[#8a6d35]">{label}</p>
